...
from .forms import TaskAddForm, KanbanAddForm, TaskAssignForm, TaskReviewForm, TaskDoneForm, TaskOverdueForm
...
class TaskAssignView(UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form): # в модель
        ...
        if datetime_deadline < datetime_now:
            form.add_error('datetime_deadline', 'Срок выполнения не может быть в прошлом')
            return self.form_invalid(form)
    ...
    def handle_no_permission(self) -> HttpResponseRedirect:
        return HttpResponseForbidden(
            render(self.request, 'tasks/forbidden.html', {'message': 'Вам нельзя изменять статус этой задачи'})
        )
...
class TaskReviewView(UserPassesTestMixin, UpdateView):
    model = Task
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if task.state != 'IN_PROGRESS':
            return render(
                request, 'tasks/error.html',
                {'error_message': 'На проверку можно взять '
                 'только назначенные задачи'}
            )
        return super().dispatch(request, *args, **kwargs)


class TaskDoneView(UserPassesTestMixin, UpdateView):
    model = Task
...
class TaskOverdueView(UserPassesTestMixin, UpdateView):
    model = Task
    template_name = 'tasks/task_overdue.html'
    form_class = TaskOverdueForm

    def form_valid(self, form):
        form.instance.to_overdue()
        return super().form_valid(form)
