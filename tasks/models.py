...
class Task(models.Model):
    ...
    datetime_deadline = models.DateTimeField(null=True, blank=True)
    executor = models.ForeignKey(User, related_name='assigned_tasks', on_delete=models.SET_NULL, null=True, blank=True)
    ...
    def to_assigned(self):
        ...
        if self.state not in ('PLANNED', 'REVIEW'):
            raise ValidationError('Назначить можно только планируемую задачу или задачу на проверке')
